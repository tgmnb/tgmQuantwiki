---
title: Infinity-Parser2 Technical Report
created: 2026-07-16
updated: 2026-07-16
type: concept
tags: [factor, 强化学习-rl, strategy, quant]
sources: [Infinity-Parser2 Technical Report]
confidence: medium
---

# Infinity-Parser2 Technical Report

> 来源：[arXiv:2607.07836v3](https://arxiv.org/abs/2607.07836v3) | 作者：

## 基本信息

| 字段 | 值 |
|------|----|
| 发表 | 2026-07-08 |
| 方法 | 强化学习/RL |
| 策略类型 | 未识别 |
| 资产类别 | 未特定 |

## 摘要

We present Infinity-Parser2, a large multimodal model that couples a controllable data-synthesis pipeline with multi-task reinforcement learning for end-to-end document parsing, addressing the persistent scarcity of faithfully annotated parsing corpora. Our contributions are threefold. First, we build a scalable synthesis engine, pairing a controllable rendering framework with an iterative refinement loop, and use it to construct and open-source Infinity-Doc2-5M: a 5-million-sample bilingual (Ch

## 核心方法论

**方法：** 强化学习/RL
**策略方向：** 未识别

## 关键发现

- We present Infinity-Parser2, a large multimodal model that couples a controllable data-synthesis pipeline with multi-task reinforcement learning for end-to-end document parsing, addressing the persistent scarcity of faithfully annotated parsing corpora.
- Our contributions are threefold.
- First, we build a scalable synthesis engine, pairing a controllable rendering framework with an iterative refinement loop, and use it to construct and open-source Infinity-Doc2-5M: a 5-million-sample bilingual (Ch.

## 实践要点

- 细节需阅读原文确认

## 相关概念

- [[strategy-prototypes]] — 策略原型
- [[risk-management]] — 风险管理
- [[factor-research]] — 因子研究框架
